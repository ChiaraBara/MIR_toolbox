...
% performs linear regression of the present state of N target processes
% from the past states of M driver processes
...
% Am, Su: VAR model parameters (theoretical or estimated with mir_idVAR)

function [SigmaY,SigmaY_X,SigmaS,SigmaX,SigmaYX,B,R] = MIR_LinReg(Am,Su,q,j,i)

N=length(j);
M=length(i);

R = MIR_Yule(Am,Su,q); % Lambda(0),...,Lambda(q) - dim: M*M*(q+1)

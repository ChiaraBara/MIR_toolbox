...
function ret = MIR_MIRdec_lin(Y,m,tau,q)

out = MIR_idVAR(Y,[1,2],[1,2],1:tau:m);

[SigmaS,SigmaS_S] = MIR_LinReg(out.eA',out.es2u,q,[1 2],[1 2]); % bivariate model, [Ypresent Xpresent] given [Ypast Xpast]
[~,SigmaX_XY] = MIR_LinReg(out.eA',out.es2u,q,1,[1 2]); % full model, Ypresent given [Ypast Xpast]
[~,SigmaY_XY] = MIR_LinReg(out.eA',out.es2u,q,2,[1 2]); % full model, Xpresent given [Ypast Xpast]

[SigmaY,SigmaY_Y] = MIR_LinReg(out.eA',out.es2u,q,2,2); % restricted model, Ypresent given Ypast
[SigmaX,SigmaX_X] = MIR_LinReg(out.eA',out.es2u,q,1,1); % restricted model, Xpresent given Xpast
